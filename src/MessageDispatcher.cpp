...
MessageDispatcher::MessageDispatcher() : taskHandle(nullptr), messageQueue(nullptr) {
    messageQueue = xQueueCreate(10, sizeof(uint32_t)); // Queue with capacity for 10 messages
}

void MessageDispatcher::startTask(const char *taskName, uint16_t stackSize, UBaseType_t priority) {
    xTaskCreate(
        taskFunction,    // Static task function
...
void MessageDispatcher::registerCallback(std::function<void(uint32_t)> callback) {
    // Register a callback
