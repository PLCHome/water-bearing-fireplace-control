...
uint16_t gpioDio::getDOVals()
{
    return (this->dir==DIR_OUTPUT)?1:0;
}

uint16_t gpioDio::getDIVals()
{
    return (this->dir==DIR_INPUT)?1:0;
}

bool gpioDio::processDoValues()
{
    bool changed = false;
    if (this->active && (this->dir==DIR_OUTPUT) && master->getGpio()->isActive())
    {
...
{
    bool changed = false;
    if (this->active && (this->dir==DIR_INPUT) && master->getGpio()->isActive())
    {
