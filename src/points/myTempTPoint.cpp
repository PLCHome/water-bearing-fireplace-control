 ...
 */
myTempTPoint::myTempTPoint(JsonVariant json, pointTyp type)
    : myPoint(json, type) {
...
/*
 * @brief Calculates the state of the temperature-dependent point based on two
 * temperature inputs.
 *
 * This method updates the state of a temperature point (`on` or `off`) based on
 * two temperature inputs (`t` and `t2`) and their corresponding thresholds
 * (`t2plus`, `t2minus`). The logic supports both standard and inverted
 ...
 * Behavior Table:
 *
 * | t  (Temperature) | t2 (Reference Temp) | t2plus | t2minus | t2p (t2 +
 * t2plus) | t2m (t2 + t2minus) | this->on Before | this->on After | Notes |
 * |------------------|---------------------|--------|---------|-------------------|--------------------|-----------------|----------------|-----------------------------------------|
 * | 2000             | 1500               | 500    | -500    | 2000 | 1000 |
 * TP_OFF          | TP_ON          | t >= t2p, turns on                     |
 * | 2000             | 1500               | 500    | -500    | 2000 | 1000 |
 * TP_ON           | TP_ON          | State remains on                       |
 * | 2000             | 1500               | -500   | 500     | 1000 | 2000 |
 * TP_OFF          | TP_OFF         | t <= t2m, stays off                    |
 * | 2000             | 1500               | -500   | 500     | 1000 | 2000 |
 * TP_ON           | TP_OFF         | t >= t2m, turns off                    |
 * | -2000            | -1500              | 500    | -500    | -1000 | -2000 |
 * TP_OFF          | TP_ON          | Negative temp: t >= t2p, turns on      |
 * | -2000            | -1500              | 500    | -500    | -1000 | -2000 |
 * TP_ON           | TP_ON          | Negative temp: State remains on        |
 * | -2000            | -1500              | -500   | 500     | -2000 | -1000 |
 * TP_OFF          | TP_OFF         | Negative temp: t <= t2m, stays off     |
 * | -2000            | -1500              | -500   | 500     | -2000 | -1000 |
 * TP_ON           | TP_OFF         | Negative temp: t >= t2m, turns off     |
 */

void myTempTPoint::calcVal() {
  ergPoint calc = this->on; ///< Store the current state (on or off)

  // Check if the temperature positions are valid indices in the tempHoldingReg
  // array
  if (this->tpos >= 0 && this->tpos < datacare.getLenTemeratures() &&
      this->tpos2 >= 0 && this->tpos2 < datacare.getLenTemeratures()) {
    // Retrieve the temperature values from the tempHoldingReg array
    int16_t t =
        datacare.getTemeratures()[this->tpos]; ///< Current temperature at tpos
    int16_t t2 =
        datacare
            .getTemeratures()[this->tpos2]; ///< Reference temperature at tpos2
    ...
    if (t2m <= t2p) ///< Standard logic: t2m is less than or equal to t2p
    {
      if (this->on != TP_ON && t >= t2p) {
        calc =
            TP_ON; ///< Turn on if temperature is greater than or equal to t2p
    ...
    } else ///< Inverted logic: t2m is greater than t2p
    {
      if (this->on != TP_ON && t <= t2p) {
        ...
        calc =
            TP_OFF; ///< Turn off if temperature is greater than or equal to t2m
      }
    }
  }
