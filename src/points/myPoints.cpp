...
void myPoints::setChanged() { this->changed = true; }

String myPoints::getJSONValue(bool obj) {
  JsonDocument doc;
  JsonObject data =
      (obj) ? doc["process"].to<JsonObject>() : doc.to<JsonObject>();
...
void myPoints::init() {
  messagedispatcher.registerCallback(
      [this](uint32_t message) { this->calcVal(message); });
  this->build();
}
  ...
  if (!SPIFFS.begin()) {
    ...
    if (!file) {
      ...
      if (error) {
        ...
        Serial.println();
        Serial.println(String("Prüfe Typ: ") +
                       String(array[2]["type"].as<String>()));
        for (auto value : array) {
          serializeJson(value, Serial);
          Serial.println();
          myPoint *next = NULL;
          Serial.println(String("Prüfe Typ: ") +
                         String(value["type"].is<pointTyp>()));
